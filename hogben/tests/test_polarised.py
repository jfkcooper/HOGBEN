"""Unit tests covering polarised code paths: SimulateReflectivity and Fisher.from_sample

These tests are designed to be fast by mocking instrument data and RNG calls.
"""
...
def test_fisher_from_sample_with_polarised_sample(monkeypatch):
    """Fisher.from_sample should construct a Fisher object from a sample,
    call SimulateReflectivity.simulate for each model, and return a valid
    Fisher information matrix of expected shape.

    We mock SimulateReflectivity.simulate to return a minimal, well-formed
    dataset so this test is fast and deterministic.
    """

    # Prepare a minimal dataset to be returned by SimulateReflectivity.simulate
    ...
    # Patch the simulate method used by Fisher.from_sample to return our data
    monkeypatch.setattr(
        'hogben.utils.SimulateReflectivity.simulate', lambda self: simulated_stack
    )

    class DummySample:
        def get_models(self):
            # The actual model object is not used because simulate() is mocked
            return [object()]

        def get_param_by_attribute(self, attr):
            # Return a single refnx Parameter with bounds so Fisher._get_bounds
...
def test_simulate_reflectivity_handles_channel_counts(monkeypatch, channels):
    """Ensure SimulateReflectivity.simulate handles models that return either a
    single reflectivity vector (channels=1) or multiple spin channels
    stacked (channels=2). The function should return an array whose number of
    rows equals 4 * channels (q, r, dr, counts) per channel and columns equal
    the number of q points per angle.
    """

    # Patch instrument flux: small deterministic table (wavelength, flux)
    ...
    # Define a model that returns either a 1D reflectivity or stacked channels
    class DummyModel:
        def __call__(self, q):
...
def test_incident_flux_called_with_polarised_flag(monkeypatch):
    """Ensure the instrument flux loader is called with the same polarised
    flag passed to SimulateReflectivity.simulate."""
    ...
    monkeypatch.setattr(
        'hogben.simulate.SimulateReflectivity._incident_flux_data', fake_incident
    )
